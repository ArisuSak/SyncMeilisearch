...
func SendStandbyStatusUpdate(_ context.Context, conn *pgconn.PgConn, ssu StandbyStatusUpdate) error {
	...
	data = AppendUint64(data, uint64(ssu.WALWritePosition))
	data = AppendUint64(data, uint64(ssu.WALFlushPosition))
	data = AppendUint64(data, uint64(ssu.WALApplyPosition))
	data = AppendInt64(data, timeToPgTime(ssu.ClientTime))
