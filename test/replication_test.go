...
loop:
	for timeout := time.After(15 * time.Second); ; {
		...
		if time.Now().After(nextStandbyMessageDeadline) {
			err := postgres.SendStandbyStatusUpdate(context.Background(), conn, postgres.StandbyStatusUpdate{WALWritePosition: clientXLogPos})
			if err != nil {
		...
		switch msg.Data[0] {
		case postgres.PrimaryKeepaliveMessageByteID:
			pkm, err := postgres.ParsePrimaryKeepaliveMessage(msg.Data[1:])
			if err != nil {
